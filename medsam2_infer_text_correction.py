...
def main():
    parser = argparse.ArgumentParser("MedSAM2 基于文本提示的二元分割评估脚本（带集中迭代纠错功能）")
    parser.add_argument('--data_dir', type=str, default='/staff/wangtiantong/SAM2_new/dataset/ACDC', help="数据集的根目录。")
    parser.add_argument('--pred_save_dir', type=str, default="./MedSAM2_text_results/ACDC/iterative_correction_focused", help="保存分割结果的目录。")
    parser.add_argument('--checkpoint', type=str, default="/staff/wangtiantong/MedSAM2/exp_log/all_dataset_20250723-1712/checkpoints/checkpoint.pt", help="模型检查点文件的路径。")
    parser.add_argument('--cfg', type=str, default="/configs/sam2.1_hiera_t512_text_predict.yaml", help="模型配置文件的路径。")
    parser.add_argument('--use_largest_cc', action='store_true', default=False, help="应用最大连通分量后处理。")
    parser.add_argument('--three_prompt', action='store_true', default=False, help="在第一帧、最后一帧和关键帧上使用文本提示。")
    parser.add_argument('--if_visualize', action='store_true', default=False, help="为每个切片保存可视化图像。")
    ...
    print("加载 MedSAM2 模型 (Text Prompts)...")
    ...
    for sample_id in tqdm(sample_ids, desc="测试3D样本"):
        ...
        text_prompts_per_class = [["" for _ in range(num_slices)] for _ in range(num_classes)]
        for i, si in enumerate(slices_info):
            for ann in si.get('annotations', []):
                cname = ann['mask_file'].split('_')[-1].replace('+', ' ').rsplit('.', 1)[0].lower().strip()
                cid = name2id.get(cname, 0)
                if cid > 0:
                    class_idx = all_class_ids.index(cid)
                    text_prompts_per_class[class_idx][i] = ann['sentences'][0]
        ...
        existing_indices = [(idx, cid) for idx, cid in enumerate(all_class_ids) if np.any(gt_4D[idx] > 0)]
        ...
        for class_idx, cid in existing_indices:
            ...
            start, end = nonzero_slices[0], nonzero_slices[-1]
            
            slice_indices = [kf]
            if args.three_prompt:
                slice_indices = sorted(list(set([kf, start, end])))
            
            print(f"  类别 '{cname}' 在帧 {slice_indices} 上进行提示")

            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                for idx in slice_indices:
                    prompt_text = text_prompts_per_class[class_idx][idx]
                    if prompt_text:
                        predictor.add_new_text(st, frame_idx=idx, obj_id=1, text_prompt=prompt_text)
            ...
            if args.if_visualize or dice_score <= 0.5:
    ...
    if results:
        ...
        csv_path = os.path.join(args.pred_save_dir, 'iterative_correction_results_focused.csv')
